...
} = useWorkflowDataRegistry(nodes, startEntitySlug, edges);

const upstreamDataSources = computed(() => {
  const targetId = selectedNode.value?.id;
  if (!targetId) return dataSources.value;
  ...
  return dataSources.value.filter((source) => upstream.has(source.nodeId));
});
...
async function enrichBeforeSave(exported: {
  nodes: any[];
  connections: any[];
  ...
  const fetchPromises = dataSources.value
    .filter((ds) => ds.kind === "entity")
    .map((ds) => fetchEntitySchema(ds.entitySlug));
  await Promise.all(fetchPromises);

  const enrichedNodes = exported.nodes.map((node) => {
    if (node.type === "notification") {
      const target = dataSources.value.find(
        (ds) => ds.nodeId === node.parameters?.targetSourceNodeId,
      );
      return {
        ...node,
        parameters: {
    ...
    if (node.type === "set_data") {
      const assignments = (node.parameters?.assignments ?? []).map(
        (assignment: any) => ({
          ...assignment,
          tokens: (assignment.tokens ?? []).map((token: any) => {
            ...
            const source = dataSources.value.find(
              (ds) => ds.nodeId === token.sourceNodeId,
            );
            const field = source?.fields.find(
              (f) => f.column_name === token.fieldSlug,
            );
...
onMounted(async () => {
  if (props.initialNodes?.length || props.initialConnections?.length) {
    loadWorkflow(
      normalizeLegacyEmailNodes(props.initialNodes ?? []),
...
const {
  ...
  fetchIntegrations: fetchSmtpIntegrations,
...
function workflowValueComplete(value: any) {
  ...
  return (
    value.sourceType === "node_output" &&
    !!value.sourceNodeId &&
    !!value.sourceFieldSlug
  );
}

function workflowValueConfigured(value: any, allowEmptyStatic = false) {
  if (!value || typeof value !== "object") return false;
  if (value.sourceType === "static") {
    return allowEmptyStatic || String(value.value ?? "").trim().length > 0;
  }
  return (
    value.sourceType === "node_output" &&
    !!value.sourceNodeId &&
...
function findListSourceReferences(
  value: unknown,
  listSourceIds: Set<string>,
  ...
  if (Array.isArray(value)) {
    for (const item of value)
      findListSourceReferences(item, listSourceIds, found);
    return found;
  }
...
async function save() {
  const triggerNodes = nodes.value.filter((n) =>
    TRIGGER_TYPES.has(n.data?.nodeType),
  );
  ...
  const dataSourceByNodeId = new Map(
    dataSources.value.map((ds) => [ds.nodeId, ds]),
  );
  ...
  const emailNodes = nodes.value.filter(
    (node) => node.data?.nodeType === "email",
  );
  if (emailNodes.length > 0) await fetchSmtpIntegrations(true);
  const activeIntegrationIds = new Set(
    smtpIntegrations.value
      .filter((item) => item.is_active)
      .map((item) => item.id_integration),
  );
  ...
  for (const node of emailNodes) {
    ...
    for (const [key, label] of [
      ...
      if (key === "to" && value.sourceType === "static") {
        ...
        if (
          ...
          toast.add({
            ...
            description:
              "Catre trebuie sa fie o singura adresa de email valida.",
            color: "error",
          });
          return;
        }
      ...
      if (value.sourceType === "node_output") {
        ...
        if (
          !source ||
          source.cardinality === "list" ||
          !hasUpstreamPath(value.sourceNodeId, node.id)
        ) {
          toast.add({
  ...
  for (const node of nodes.value.filter((node) =>
    wordNodeTypes.has(node.data?.nodeType),
  )) {
    const nodeType = String(node.data?.nodeType);
    const params = node.data?.parameters ?? {};
    if (nodeType !== "word_open") {
      const documentSource = dataSourceByNodeId.get(
        String(params.documentSourceNodeId ?? ""),
      );
      if (
        !documentSource ||
        documentSource.kind !== "document" ||
        documentSource.documentPackage !== "word" ||
        documentSource.cardinality === "list" ||
        !hasUpstreamPath(documentSource.nodeId, node.id)
      ) {
        toast.add({
          title: `Document Word invalid in nodul "${node.data.label}"`,
          description:
            "Alege un document Word produs de un nod anterior single/item.",
          color: "error",
        });
        return;
      }
    ...
    if (nodeType === "word_open")
      requiredValues.push([params.fileId, "Fisier"]);
    if (
      [
        "word_replace_text",
        "word_create_table_rows",
      ...
      requiredValues.push([params.search, "Cauta"]);
    }
    if (nodeType === "word_replace_text")
      requiredValues.push([params.replace, "Inlocuieste cu", true]);
    if (
      ["word_create_table_rows", "word_insert_table_rows"].includes(nodeType)
    ) {
      requiredValues.push([params.nrOfNewRows, "Numar randuri"]);
    }
    if (nodeType === "word_save")
      requiredValues.push([params.fileName, "Nume fisier"]);
    if (nodeType === "word_update")
      requiredValues.push([params.fileId, "Fisier"]);

    for (const [value, label, allowEmpty] of requiredValues) {
      if (!workflowValueConfigured(value, allowEmpty)) {
        toast.add({
          title: `Configuratie incompleta in nodul "${node.data.label}"`,
          description: `Campul "${label}" trebuie sa aiba o valoare fixa sau un camp upstream.`,
          color: "error",
        });
        return;
      }
      if (value.sourceType === "node_output") {
        ...
        if (
          ...
          toast.add({
            title: `Sursa invalida in nodul "${node.data.label}"`,
            description: `Campul "${label}" trebuie sa vina dintr-un nod anterior single/item.`,
            color: "error",
          });
          return;
        }
    ...
    const rowValue = params.nrOfNewRows;
    if (
      ["word_create_table_rows", "word_insert_table_rows"].includes(nodeType) &&
      rowValue?.sourceType === "static"
    ) {
      const count = Number(rowValue.value);
      if (!Number.isInteger(count) || count < 1 || count > 1000) {
        toast.add({
          title: `Numar de randuri invalid in nodul "${node.data.label}"`,
          description: "Valoarea trebuie sa fie un intreg intre 1 si 1000.",
          color: "error",
        });
        return;
      }
  ...
  for (const node of nodes.value.filter(
    (node) => node.data?.nodeType === "notification",
  )) {
    const params = node.data?.parameters ?? {};
    const recipient = params.recipient ?? {};
    const recipientComplete =
      recipient.sourceType === "static"
        ? !!recipient.profileId
        : !!recipient.sourceNodeId && !!recipient.sourceFieldSlug;

    if (
      !recipientComplete ||
      !templateHasContent(params.subjectTokens ?? []) ||
      !templateHasContent(params.contentTokens ?? [])
    ) {
      toast.add({
        title: `Configuratie incompleta in nodul "${node.data.label}"`,
        description:
          "Completeaza destinatarul, subiectul si continutul notificarii.",
        color: "error",
      });
      return;
    }
    ...
    const sourceIds = new Set<string>([
      ...
      ...(params.subjectTokens ?? [])
        .map((token: any) => token.sourceNodeId)
        .filter(Boolean),
    ...
    for (const sourceId of sourceIds) {
      ...
      if (
        !source ||
        source.cardinality === "list" ||
        !hasUpstreamPath(sourceId, node.id)
      ) {
        toast.add({
          title: `Sursa invalida in nodul "${node.data.label}"`,
          description:
            'Fiecare camp trebuie sa vina dintr-un nod anterior single/item. Pentru liste foloseste "Pentru Fiecare".',
          color: "error",
        });
        return;
      }
  ...
  for (const node of nodes.value) {
    ...
    if (!source || source.cardinality !== "list") {
      toast.add({
        ...
        description:
          'Alege un nod "Citeste Inregistrari" care intoarce mai multe inregistrari.',
        color: "error",
      });
      return;
    }

    const hasSourceEdge = edges.value.some(
    ...
    if (!hasSourceEdge) {
      toast.add({
        ...
        description:
          'Conecteaza nodul care citeste lista direct la nodul "Pentru fiecare".',
        color: "error",
      });
      return;
    }
  ...
  for (const node of nodes.value) {
    ...
    const refs = findListSourceReferences(
    ...
    toast.add({
      ...
      description:
        'Listele trebuie parcurse prin nodul "Pentru fiecare" inainte sa alegi campuri din ele.',
      color: "error",
    });
    return;
  }
...
<template>
  <div class="flex h-full w-full">
    ...
    <WorkflowPanelsNodeConfigPanel
      :node="selectedNode"
      :data-sources="upstreamDataSources"
