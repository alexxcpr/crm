...
const allUiTypeOptions = [
  ...
  { label: 'Suma (moneda)', value: 'currency' },
...
const state = reactive({
  ...
  currency_code: (props.field?.validation_rules?.currency_code as string) ?? '',
...
async function onSubmit() {
  ...
  try {
    ...
    if (showCurrencyFields.value && state.currency_code) {
      validationRules.currency_code = state.currency_code
    }
...
<template>
  ...
  >
    ...
    <div v-if="showCurrencyFields" class="space-y-4">
      <USeparator />
      <h4 class="text-sm font-semibold text-muted uppercase tracking-wider">
        Moneda
      </h4>
      <UFormField
        label="Moneda campului"
        name="currency_code"
        description="Lasa necompletat pentru a folosi moneda implicita a tenantului."
      >
        <USelect
          v-model="state.currency_code"
          :items="[
            { label: 'Implicita tenant', value: '' },
            { label: 'RON', value: 'RON' },
            { label: 'EUR', value: 'EUR' },
            { label: 'USD', value: 'USD' },
            { label: 'GBP', value: 'GBP' }
