...
describe('SmtpMailService', () => {
  ...
  beforeEach(() => {
    ...
    ).mockReturnValue({ sendMail, close });
  ...
  it('trimite text simplu cu setarile SMTP configurate', async () => {
    ...
    const result =
      await service.sendWorkflowEmail(
        'integration-id',
        'client@example.com',
        'Subiect',
        'Continut',
  ...
  it('opreste trimiterea cand deadline-ul workflow-ului este anulat', async () => {
    const controller = new AbortController();
    sendMail.mockReturnValueOnce(
      new Promise(() => undefined),
    );
    const service = new SmtpMailService(
      integrations as any,
    );
    const sending = service.sendWorkflowEmail(
      'id',
      'a@example.com',
      'Subiect',
      'Continut',
