...
export class IntegrationsService {
  ...
  async list(type = 'smtp') {
    if (type !== 'smtp')
      throw new BadRequestException(
        `Tipul de integrare "${type}" nu este disponibil.`,
      );
    const rows = await this.knex<IntegrationRow>(
    ...
    return {
      data: rows.map((row) =>
        this.publicRow(
          row,
          usages.get(row.id_integration) ?? 0,
        ),
      ),
    };
  }
  ...
  async createSmtp(
    ...
    const config = this.configFromDto(
      dto,
      username,
    );
    this.assertTransportConfiguration(config);

    const [row] = await this.knex<IntegrationRow>(
      'integration_definition',
    )
      .insert({
        type: 'smtp',
  ...
  async update(
    ...
    const username =
      dto.username === undefined
        ...
        : this.optional(dto.username);
    const suppliedPassword = this.optional(
      dto.password,
    );
    const clearPassword =
      dto.clearPassword === true;
    const hasPassword = clearPassword
      ? false
      : !!suppliedPassword ||
    ...
    const config: StoredSmtpConfig = {
      host:
        dto.host?.trim() ?? currentConfig.host,
      port: dto.port ?? currentConfig.port,
      security:
        dto.security ?? currentConfig.security,
      username,
      fromName:
        dto.fromName === undefined
          ? currentConfig.fromName
          : this.optional(dto.fromName),
      fromEmail:
        dto.fromEmail?.trim().toLowerCase() ??
        currentConfig.fromEmail,
      rejectUnauthorized:
        dto.rejectUnauthorized ??
        currentConfig.rejectUnauthorized,
    };
    this.assertTransportConfiguration(config);
    ...
    if (clearPassword)
      ...
      patch.secret_encrypted =
        this.crypto.encrypt(suppliedPassword);
    ...
    const [row] = await this.knex<IntegrationRow>(
      ...
      .returning('*');
    const usage =
      (await this.usageCounts()).get(id) ?? 0;
  ...
  async remove(
    ...
    const current = await this.findRow(id, false);
    const affected =
      await this.workflowsUsing(id);
    let replacement: IntegrationRow | null = null;

    if (affected.length > 0) {
      if (!replacementIntegrationId) {
        throw new ConflictException(
          'Integrarea este folosita de workflow-uri. Alege o integrare SMTP inlocuitoare.',
        );
      }
      if (replacementIntegrationId === id)
        throw new BadRequestException(
          'Integrarea nu se poate inlocui cu ea insasi.',
        );
      replacement = await this.findRow(
        replacementIntegrationId,
        false,
      );
      if (
        ...
        throw new BadRequestException(
          'Integrarea inlocuitoare trebuie sa fie un cont SMTP activ.',
        );
      }
      ...
      await this.knex.transaction(async (trx) => {
          ...
          const nodes = this.parseNodes(
            ...
            return {
              ...
              parameters: {
                ...node.parameters,
                integrationId:
                  replacement!.id_integration,
                integrationName:
                  replacement!.name,
              },
            };
          });
          const compiledIr = this.parseObject(
            workflow.compiled_ir,
          );
          if (Array.isArray(compiledIr?.nodes)) {
            compiledIr.nodes =
              compiledIr.nodes.map((node: any) =>
                node.type === 'email' &&
                node.config?.integrationId === id
                  ? {
                      ...node,
                      config: {
                        ...node.config,
            ...
            compiledIr.dependencies ??= {
              entityIds: [],
              fieldIds: [],
              integrationIds: [],
              httpDomains: [],
            };
            compiledIr.dependencies.integrationIds =
              [
                ...new Set(
                  (
                    compiledIr.dependencies
        ...
        await trx('integration_definition')
          .where('id_integration', id)
          .update({
            is_active: false,
            id_replaced_by:
      ...
      await this.knex('integration_definition')
        .where('id_integration', id)
        .update({
          is_active: false,
          date_deleted: new Date(),
    ...
    return {
      message: affected.length
        ? `${affected.length} workflow-uri au fost mutate pe integrarea noua.`
        : 'Integrarea a fost stearsa.',
    };
  }

  async findSmtpForSending(
    id: string,
  ): Promise<{
    row: IntegrationRow;
    ...
    while (
      ...
      if (visited.has(row.id_integration))
        throw new BadRequestException(
          'Redirect circular intre integrari.',
        );
    ...
    if (row.date_deleted || !row.is_active)
      throw new BadRequestException(
        `Integrarea SMTP "${row.name}" este inactiva.`,
      );
    return {
      row,
      config: this.parseConfig(row.config),
      password: row.secret_encrypted
        ? this.crypto.decrypt(
  ...
  private async findRow(
    ...
    const query = this.knex<IntegrationRow>(
      'integration_definition',
    ).where('id_integration', id);
    if (!includeDeleted)
    ...
    if (!row)
      throw new NotFoundException(
        'Integrarea nu a fost gasita.',
      );
    if (row.type !== 'smtp')
      throw new BadRequestException(
        'Integrarea selectata nu este SMTP.',
      );
    return row;
  }

  private configFromDto(
    dto: CreateSmtpIntegrationDto,
    username: string | null,
  ): StoredSmtpConfig {
    return {
  ...
  private assertAuthentication(
    ...
    if (
      !!username !== (!!password || hasPassword)
    ) {
      throw new BadRequestException(
        'Username-ul si parola SMTP trebuie completate impreuna sau lasate ambele goale.',
      );
    }
  }

  private assertTransportConfiguration(
    config: StoredSmtpConfig,
  ) {
    if (
      config.port === 465 &&
      ...
      throw new BadRequestException(
        'Portul 465 necesita securitate TLS / SSL direct.',
      );
    }
    ...
    if (
      ...
      if (!validGmailPair) {
        throw new BadRequestException(
          'Pentru smtp.gmail.com foloseste portul 465 cu TLS / SSL direct sau portul 587 cu STARTTLS.',
        );
      }
    }
  }

  private async workflowsUsing(
    integrationId: string,
  ): Promise<any[]> {
    ...
    return workflows.filter((workflow) =>
      this.parseNodes(workflow.source_nodes).some(
        (node) =>
          node.type === 'email' &&
          node.parameters?.integrationId ===
            integrationId,
  ...
  private async usageCounts(): Promise<
    Map<string, number>
  > {
    ...
    for (const workflow of workflows) {
      ...
      for (const node of this.parseNodes(
        ...
        if (
          ...
          ids.add(node.parameters.integrationId);
      }
      for (const id of ids)
