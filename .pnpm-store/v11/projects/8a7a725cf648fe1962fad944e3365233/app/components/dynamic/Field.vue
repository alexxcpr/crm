...
const fieldCurrency = computed(() =>
  props.field.validation_rules?.currency_code
  || branding.value.defaultCurrency
)
...
// Placeholder texts
const datePlaceholder = computed(() => branding.value.dateFormat
  .replace('dd', 'zz')
  .replace('MM', 'll')
  .replace('yyyy', 'aaaa'))
const dateTimePlaceholder = computed(() => `${datePlaceholder.value} hh:mm:ss`)
...
function parseDateFromText(text: string): DateValue | null {
  ...
  const format = branding.value.dateFormat as keyof typeof patterns
  const match = trimmed.match(patterns[format] || patterns['dd.MM.yyyy'])
  if (match) {
    const values = match.slice(1).map(part => parseInt(part, 10))
...
function formatDateForDisplay(dv: any): string {
  if (!dv) return ''
  return branding.value.dateFormat
    .replace('dd', String(dv.day).padStart(2, '0'))
    .replace('MM', String(dv.month).padStart(2, '0'))
    .replace('yyyy', String(dv.year))
}
...
const parseToCalendarDateTime = (val: string): DateValue | undefined => {
  try {
    ...
    return fromDate(date, branding.value.timezone)
  } catch {
    return undefined
  }
}
...
function parseDateTimeFromText(text: string): ParsedDateTime | null {
  ...
  const [datePart, timePart, extra] = trimmed.split(/\s+/)
  if (!datePart || extra) return null
...
function emitDateOnlyValue() {
  ...
  emit('update:modelValue', new CalendarDateTime(
    date.year,
    date.month,
    date.day,
    0,
    0,
    0
  ).toDate(branding.value.timezone).toISOString())
}
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <template #trailing>
          <span class="text-dimmed text-sm ml-2">{{ fieldCurrency }}</span>
        </template>
      </UInput>
