...
const dateFormatter = computed(() => new Intl.DateTimeFormat(branding.value.locale, {
  day: '2-digit',
  month: '2-digit',
  year: 'numeric',
  timeZone: branding.value.timezone
}))
const dateTimeFormatter = computed(() => new Intl.DateTimeFormat(branding.value.locale, {
  day: '2-digit',
  month: '2-digit',
  year: 'numeric',
  hour: '2-digit',
...
const valueFormatter = computed(() => new Intl.NumberFormat(branding.value.locale, props.widget.value_format === 'currency'
  ? { style: 'currency', currency: props.widget.currency_code || branding.value.defaultCurrency, maximumFractionDigits: 2 }
...
function pointLabel(key: string, fallback: string, includeTime = false) {
  ...
  return (includeTime ? dateTimeFormatter.value : dateFormatter.value).format(date).replace(',', '')
}
...
const donutTooltipTriggers = computed(() => ({
  [VisDonutSelectors.segment]: (arc: { data: DashboardChartPoint }) => {
    ...
    const percent = total ? ` (${new Intl.NumberFormat(branding.value.locale, { maximumFractionDigits: 1 }).format(arc.data.value / total * 100)}%)` : ''
