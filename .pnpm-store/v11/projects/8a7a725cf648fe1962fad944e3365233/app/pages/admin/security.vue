...
type AccessLevel = 'platform_owner' | 'tenant_admin' | 'user'
type Profile = { id_profile: string, username: string, email: string, display_name: string | null, access_level: AccessLevel, is_active: boolean, roles: Role[], roleIds: string[] }
...
const { canManageAccessLevels } = useAccess()
...
const profileForm = reactive({ id: '', loginUsername: '', displayName: '', username: '', email: '', roleIds: [] as string[], accessLevel: 'user' as AccessLevel, originalAccessLevel: 'user' as AccessLevel, isActive: true })
...
const accessLevelOptions = [
  ...
  { label: 'Administrator client', value: 'tenant_admin' }
]
const accessLevelLabels: Record<AccessLevel, string> = {
  platform_owner: 'Platform Owner',
  tenant_admin: 'Administrator client',
  user: 'Utilizator'
}
...
function canEditProfile(profile: Profile) {
  if (profile.access_level === 'platform_owner') return false
  return profile.access_level === 'user' || canManageAccessLevels.value
}
...
async function load() {
  const [loadedUsers, loadedRoles, loadedEntities, loadedRoleGroups] = await Promise.all([
    ...
    apiFetch<Entity[]>('/v1/admin/security/entity-catalog'),
...
function openProfileEditor(profile: Profile & { login_username?: string }) {
  ...
  profileForm.accessLevel = profile.access_level
  profileForm.originalAccessLevel = profile.access_level
...
async function saveProfileForm() {
  ...
  try {
    ...
    if (
      canManageAccessLevels.value
      && profileForm.accessLevel !== profileForm.originalAccessLevel
      && profileForm.accessLevel !== 'platform_owner'
    ) {
      await apiFetch(`/v1/admin/security/profiles/${profileForm.id}/access-level`, {
        method: 'PUT',
        body: { accessLevel: profileForm.accessLevel }
      })
    }
    await load()
      ...
      <div class="overflow-hidden rounded-md border border-default">
        ...
        <div v-for="profile in filteredProfiles" :key="profile.id_profile" class="grid gap-3 border-b border-default px-4 py-3 last:border-b-0 xl:grid-cols-[minmax(16rem,1.4fr)_minmax(10rem,.75fr)_minmax(14rem,1fr)_7rem_auto] xl:items-center">
          ...
          <div class="flex min-w-0 flex-wrap gap-1.5">
            <UBadge
              :label="accessLevelLabels[profile.access_level]"
              :color="profile.access_level === 'platform_owner' ? 'primary' : profile.access_level === 'tenant_admin' ? 'warning' : 'neutral'"
              variant="subtle"
              size="md"
            />
            <UBadge
    ...
    >
      <template #body>
        <div class="space-y-5">
          ...
          <UFormField v-if="canManageAccessLevels && profileForm.originalAccessLevel !== 'platform_owner'" label="Nivel de acces">
            <USelect
              v-model="profileForm.accessLevel"
