...
function actions(dashboard: DashboardDefinition) {
  return [[{
    ...
    to: `/builder/dashboards/${dashboard.id_ui_dashboard}`
  }], [{
    label: 'Dezactiveaza',
...
<template>
  ...
  <div v-else>
    <div class="mb-4 flex items-center justify-between gap-4">
      ...
      <UButton label="Dashboard nou" icon="i-lucide-plus" to="/builder/dashboards/new" />
    </div>
    ...
    >
      <UTable :data="dashboards" :columns="columns" :loading="loading">
        ...
        <template #name-cell="{ row }">
          <NuxtLink :to="`/builder/dashboards/${row.original.id_ui_dashboard}`" class="font-medium text-primary hover:underline">
            {{ row.original.name }}
          </NuxtLink>
        </template>
