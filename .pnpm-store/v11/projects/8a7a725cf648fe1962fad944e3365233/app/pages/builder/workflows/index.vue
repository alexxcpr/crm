...
function getDropdownItems(wf: any) {
  return [[{
    ...
    onClick: () => router.push(`/builder/workflows/${wf.id_workflow}`)
  }, {
...
<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      ...
      <div class="flex items-center gap-1.5">
        ...
        <UButton
          label="Workflow nou"
          icon="i-lucide-plus"
          @click="router.push('/builder/workflows/new')"
        />
      </div>
    </div>
      ...
      >
        ...
        <template #edit-cell="{ row }">
          <UButton
            ...
            variant="ghost"
            size="xs"
            @click="router.push(`/builder/workflows/${row.original.id_workflow}`)"
          />
        </template>

        <template #name-cell="{ row }">
          <NuxtLink
            :to="`/builder/workflows/${row.original.id_workflow}`"
    ...
    <div v-if="!loading && workflows.length === 0" class="py-12">
      ...
      >
        <template #actions>
          <UButton label="Workflow nou" icon="i-lucide-plus" @click="router.push('/builder/workflows/new')" />
        </template>
      </UEmpty>
    </div>
